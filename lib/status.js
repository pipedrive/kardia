(function () {
   'use strict';
	...
	function Status(config) {
		...
		this.measureFallBehindTimeout = null;
	...
	Status.prototype.measureFallBehind = function() {
		...
		this.measureFallBehindTimeout = setTimeout(function() {
	...
	Status.prototype.startServer = function() {
		...
		this.server = http.createServer((function (req, res) {
			...
			} else if (extraEndpoints[req.url]) {
				this.serveExtraEndpoint(req, res);
			} else {
	...
	Status.prototype.serveExtraEndpoint = function(req, res) {
		var output = extraEndpoints[req.url];
		...
		zlib.gzip(output.toString(), function (_, result) {
			res.end(result);
		});
	};
	...
	Status.prototype.registerEndpoint = function(endpoint, output) {
		extraEndpoints[(endpoint.charAt(0) === '/' ? endpoint : '/' + endpoint)] = output;
	};
	...
	Status.prototype.close = function() {
		if (this.measureFallBehindTimeout) {
			clearTimeout(this.measureFallBehindTimeout);
		}
		this.stopServer();
	};
