(function () {
   'use strict';
	...
	Status.prototype.serveHealthcheckRequest = function(req, res) {
		...
		if (parsedUrl.query) {
			urlParams = querystring.parse(parsedUrl.query);
		}

		if (urlParams && urlParams.service_name && urlParams.service_name !== this.config.name) {
			return callback(new Error('Service name mismatch'));
		}
