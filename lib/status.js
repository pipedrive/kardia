(function () {
   'use strict';
	...
	Status.prototype.gauge = function(name, value) {
		if (this.gauges[name] !== value && Number(value) !== value) {
			this.gauges[name] = value;
			if (cluster.isWorker && process.send) {
				process.send({ '~kardia': { cmd: 'gauge', args: [].slice.call(arguments) } });
				return;
			}
		}
	};
    ...
    Status.prototype.reset = function(name) {
        if (cluster.isWorker && process.send) {
	...
	Status.prototype.startServer = function() {
		...
		this.server = http.createServer((function (req, res) {
			...
			} else if (req.url.substr(0, 8) === '/metrics') {
				this.serveMetricsRequest(req, res);
			} else {
	...
	Status.prototype.serveHealthcheckRequest = function(req, res) {
		...
		if (parsedUrl.query && parsedUrl.query.service_name && parsedUrl.query.service_name !== this.config.name) {
			return callback(new Error('Service name mismatch (expected ' + this.config.name + ' but ' + parsedUrl.query.service_name + ' supplied as parameter)'));
		}
	...
	Status.prototype.serveMetricsRequest = function(req, res) {
	...
	Status.prototype.generateMetrics = function() {
		var output = '';

		for (var key in this.counters) {
			var name = this.config.name.replace(/[^0-9a-zA-Z]/g, '_') + '_' + key.replace(/[^0-9a-zA-Z]/g, '_') + '_total';
			output += name + ' ' + this.counters[key] + '\n'
		}

		for (var key in this.gauges) {
			output += '# TYPE ' + key + ' gauge\n' + key + ' ' + this.gauges[key] + '_total\n'
		}

		prometheusRegistries.forEach(function(registry) {
			output += registry.metrics() + '\n';
		});
	...
	Status.prototype.addPrometheusRegistry = function(registry) {
		prometheusRegistries.push(registry);
	};
